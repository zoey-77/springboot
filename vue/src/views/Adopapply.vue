...
<!--      <el-input style="width: 200px" placeholder="请输入" suffix-icon="el-icon-position" class="ml-5" v-model="address"></el-input>-->
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9090/adoptApply/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="name" label="相关领养"></el-table-column>
      <el-table-column prop="description" label="申请说明"></el-table-column>
      <el-table-column prop="user" label="用户"></el-table-column>
      <el-table-column prop="adoptId" label="领养ID"></el-table-column>
      ...
      <el-table-column label="操作"  width="180" align="center">
        <template slot-scope="scope">
          <el-button type="success" @click="handleEdit(scope.row)" v-if="user.role != 'ROLE_USER' && scope.row.state == '待审核'">审核</el-button>
          <el-popconfirm
              class="ml-5"
              confirm-button-text='确定'
              cancel-button-text='我再想想'
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="40%" :close-on-click-modal="false">
      <el-form label-width="140px" size="small" style="width: 85%;" :model="form" :rules="rules" ref="ruleForm">
        <el-form-item prop="reviewer" label="审核人姓名">
...
export default {
  ...
  methods: {
    load() {
      this.request.get("/adoptApply/page", {
        params: {
    ...
    save() {
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            this.request.post("/adoptApply", this.form).then(res => {
              if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/adoptApply/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/adoptApply/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/adoptApply/export")
    },
