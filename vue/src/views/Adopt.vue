...
<!--      <el-input style="width: 200px" placeholder="请输入" suffix-icon="el-icon-position" class="ml-5" v-model="address"></el-input>-->
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9090/adopt/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="state" label="领养状态"></el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="60%" :close-on-click-modal="false">
      <el-form label-width="140px" size="small" style="width: 90%;" :model="form" :rules="rules" ref="ruleForm">
  ...
  methods: {
    ...
    save() {
      ...
      this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            this.request.post("/adopt", this.form).then(res => {
              if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/adopt/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/adopt/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/adopt/export")
    },
