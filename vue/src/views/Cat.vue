...
<!--      <el-input style="width: 200px" placeholder="请输入" suffix-icon="el-icon-position" class="ml-5" v-model="address"></el-input>-->
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9090/cat/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="color" label="毛色"></el-table-column>
      <el-table-column prop="state" label="状态"></el-table-column>
      <el-table-column prop="characters" label="性格"></el-table-column>
      ...
      <el-table-column prop="content" label="介绍">
        <template v-slot="scope">
          <el-button type="primary" @click="view(scope.row.content)">查看介绍</el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="60%" :close-on-click-modal="false">
      <el-form label-width="140px" size="small" style="width: 95%;" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="character" label="性格">
    ...
    <el-dialog title="介绍" :visible.sync="dialogFormVisible1" width="60%" :close-on-click-modal="false">
      <el-card>
  ...
  methods: {
    ...
    save() {
      ...
      this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            this.request.post("/cat", this.form).then(res => {
              if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/cat/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/cat/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/cat/export")
    },
